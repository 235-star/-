...
function checkRateLimit(type) {
    ...
    if (lastPostTime) {
        ...
        if (timeDiff < 30000) {
            ...
            alert(
                `レビューの投稿間隔は30秒以上空けてください。あと${remainingSeconds}秒待ってから再度お試しください。`,
            );
...
document.addEventListener("DOMContentLoaded", function () {
    ...
    function initializeApp() {
        universitiesData.forEach((u) => updateUniversityStats(u.id));
    ...
    function matchesUniversity(university, query) {
        ...
        // バリエーションがない場合は従来の方法
        return (
            normalizeInput(university.name).includes(normalizedQuery) ||
    ...
    function showUniversitySuggestions(query) {
        ...
        filteredUniversities.sort((a, b) => {
            ...
            // 名前の先頭マッチを優先
            const aStartsWithMatch = normalizeInput(a.name).startsWith(
                normalizedQuery,
            );
            const bStartsWithMatch = normalizeInput(b.name).startsWith(
                normalizedQuery,
            );
        ...
        const suggestionsHTML = filteredUniversities
            .slice(0, 6)
            .map(
                (university) => `
    ...
    function showPage(pageName) {
        ...
        const addProfessorFooter = document.getElementById(
    ...
    function updateBreadcrumb() {
        ...
        switch (currentPage) {
            ...
            case "professorDetail":
                const uniForProf = universitiesData.find(
                    (u) => u.id === currentUniversityId,
                );
                const professor = professorsData.find(
    ...
    function displayUniversities(searchQuery = "") {
        ...
        if (searchQuery) {
            ...
            filteredUniversities = universitiesData.filter((uni) => {
                ...
                return (
                    ...
                    (uni.nameVariations &&
                        uni.nameVariations.some((variation) =>
                            normalizeInput(variation).includes(normalizedQuery),
                        ))
                );
            });
            ...
            // 検索結果をソート（関連性順）
            filteredUniversities.sort((a, b) => {
                const aNameMatch = normalizeInput(a.name).includes(
                    normalizedQuery,
                );
                const bNameMatch = normalizeInput(b.name).includes(
                    normalizedQuery,
                );
        ...
        universitiesList.innerHTML += filteredUniversities
            .map(
                (university) => `
    ...
    window.showUniversityDetail = function (universityId) {
        ...
        const reviews = universityReviewsData.filter(
            (r) => r.universityId === universityId,
        );
        const professorCount = professorsData.filter(
    ...
    // 教授一覧表示
    window.showProfessorsList = function (universityId) {
        currentUniversityId = universityId;
    ...
    function displayProfessors(professors) {
        ...
        const professorsHtml = professors
            .map(
                (professor) => `
    ...
    // 教授詳細表示
    window.showProfessorDetail = function (professorId) {
    ...
    function setupEventListeners() {
        // 大学検索
        const universitySearchBtn = document.getElementById(
        ...
        // 教授検索
        const professorSearchBtn = document.getElementById(
            "professor-search-btn",
        );
        const professorSearchInput =
            document.getElementById("professor-search");
        const departmentFilter = document.getElementById("department-filter");
        ...
        function searchProfessors() {
            ...
            let professors = professorsData.filter(
                (p) => p.universityId === currentUniversityId,
            );
    ...
    function setupModalListeners() {
        ...
        // フォーム送信
        document
            .getElementById("university-review-form")
            .addEventListener("submit", submitUniversityReview);
        document
            .getElementById("professor-review-form")
            .addEventListener("submit", submitProfessorReview);
    }

    // レビューモーダル開く
    window.openUniversityReviewModal = function (universityId) {
        currentUniversityId = universityId;
        document.getElementById("university-review-modal").style.display =
            "block";

        // Check rate limiting and update submit button
        setTimeout(() => {
            updateSubmitButtonState(
                '#university-review-form button[type="submit"]',
                "university",
            );
        }, 100);
    };
    ...
    window.openProfessorReviewModal = function (professorId) {
        currentProfessorId = professorId;
        document.getElementById("professor-review-modal").style.display =
            "block";

        // Check rate limiting and update submit button
        setTimeout(() => {
            updateSubmitButtonState(
                '#professor-review-form button[type="submit"]',
                "professor",
            );
        }, 100);
    };
    ...
    // レビュー投稿
    function submitUniversityReview(e) {
        e.preventDefault();
        ...
        const overallRating =
            parseInt(
                form.querySelector('[data-rating="overall"]').dataset.value,
            ) || 0;
        const academicRating =
            parseInt(
                form.querySelector('[data-rating="academic"]').dataset.value,
            ) || 0;
        const facilityRating =
            parseInt(
                form.querySelector('[data-rating="facility"]').dataset.value,
            ) || 0;
        const employmentRating =
            parseInt(
                form.querySelector('[data-rating="employment"]').dataset.value,
            ) || 0;
        ...
        const newReview = {
            id: universityReviewsData.length + 1,
            ...
            graduationYear:
                parseInt(graduationYear) || new Date().getFullYear(),
            date: new Date().toISOString().split("T")[0],
        };

        universityReviewsData.push(newReview);
        updateUniversityStats(currentUniversityId);
    ...
    function submitProfessorReview(e) {
        e.preventDefault();
        ...
        const difficulty =
            parseInt(
                form.querySelector('[data-rating="difficulty"]').dataset.value,
            ) || 0;
        const strictness =
            parseInt(
                form.querySelector('[data-rating="strictness"]').dataset.value,
            ) || 0;
        const course = document.getElementById("course").value;
        const semester = document.getElementById("semester").value;
        const comment = document.getElementById("prof-comment").value;
        const wouldRetake = document.querySelector(
            'input[name="would-retake"]:checked',
        )?.value;
        ...
        if (!wouldRetake) {
            alert(
                "「この講義をもう一度履修したいと思いますか？」の項目を選択してください。",
            );
            return;
        }
        ...
        submitReview(
          newReview.professorId,
          newReview.comment,
          newReview.rating
        );

        reviewsData.push(newReview);
        updateProfessorStats(currentProfessorId);
    ...
    // 統計更新
    function updateUniversityStats(universityId) {
        const university = universitiesData.find((u) => u.id === universityId);
        const reviews = universityReviewsData.filter(
            (r) => r.universityId === universityId,
        );
    ...
    function updateProfessorStats(professorId) {
        const professor = professorsData.find((p) => p.id === professorId);
        const reviews = reviewsData.filter(
            (r) => r.professorId === professorId,
        );
    ...
    function openAddProfessorModal() {
        ...
        if (modal && currentUniversity) {
            ...
            setTimeout(() => {
                updateSubmitButtonState(
                    '#addProfessorForm button[type="submit"]',
                    "professor_add",
                );
            }, 100);
        } else {
            alert(
                "大学を選択してその大学の教授一覧ページから教授を追加してください。",
            );
        }
    }

    function openAddProfessorForUniversity(
        universityId,
        universityName,
        department = "",
    ) {
        console.log(
            "Opening modal for university:",
            universityId,
            universityName,
            typeof universityId,
        );
        ...
        if (modal) {
            ...
            console.log(
                "Modal data-university-id attribute:",
                modal.getAttribute("data-university-id"),
            );
        } else {
    ...
    function submitProfessorData(event) {
        ...
        if (!professorData.universityId) {
            ...
            console.error(
                "University ID is invalid:",
                professorData.universityId,
            );
            return;
        }

        const university = universitiesData.find(
            (u) => u.id === professorData.universityId,
        );
        if (!university) {
            alert("大学が見つかりません。");
            return;
