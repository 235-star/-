...
document.addEventListener('DOMContentLoaded', function() {
    ...
    async function loadStoredReviews() {
        try {
            ...
            if (data.success && data.reviews && data.reviews.length > 0) {
                // Convert stored reviews to the format expected by the frontend
                const convertedReviews = data.reviews.map(storedReview => {
                    if (storedReview.content && typeof storedReview.content === 'object') {
                        // Convert new format reviews to old format for compatibility
                        return {
                            id: storedReview.id || Date.now() + Math.random(),
                            professorId: storedReview.content.professorId || storedReview.content.professor_id,
                            universityId: storedReview.content.universityId || storedReview.content.university_id,
                            course: storedReview.content.course || '',
                            semester: storedReview.content.semester || '2024年',
                            overallRating: storedReview.content.overallRating || storedReview.content.overall_rating || 5,
                            teachingQuality: storedReview.content.teachingQuality || storedReview.content.teaching_quality || 5,
                            difficulty: storedReview.content.difficulty || 5,
                            strictness: storedReview.content.strictness || 5,
                            content: storedReview.content.content || storedReview.content.comment || '',
                            wouldRetake: storedReview.content.wouldRetake || storedReview.content.would_retake || false,
                            attendanceRequired: storedReview.content.attendanceRequired || storedReview.content.attendance_required || 'unknown',
                            grade: storedReview.content.grade || '',
                            anonymous: true,
                            date: storedReview.timestamp ? new Date(storedReview.timestamp).toISOString().split('T')[0] : new Date().toISOString().split('T')[0],
                            upvotes: 0,
                            downvotes: 0
                        };
                    }
                    return storedReview;
                }).filter(review => review.professorId || review.universityId); // Only include reviews with valid IDs
                
                // Merge with existing reviews, avoiding duplicates
                const existingIds = new Set(reviewsData.map(r => r.id));
                const newReviews = convertedReviews.filter(r => !existingIds.has(r.id));
                reviewsData.push(...newReviews);
                
                console.log(`Loaded ${newReviews.length} reviews from persistent storage`);
                
                // Update professor statistics after loading reviews
                universitiesData.forEach(u => updateUniversityStats(u.id));
            }
        } catch (error) {
            console.log('No stored reviews found or error loading:', error);
        }
    }

    async function initializeApp() {
        // Load stored reviews first
        await loadStoredReviews();
        
        // Update university stats after loading reviews
        universitiesData.forEach(u => updateUniversityStats(u.id));
        ...
        // Ensure contact info is visible on home page
        const contactInfo = document.querySelector('.contact-info-fixed');
        if (contactInfo) {
    ...
    function showPage(pageName) {
        ...
        // Show contact info only on home page (universities page)
        const contactInfo = document.querySelector('.contact-info-fixed');
        if (contactInfo) {
            if (pageName === 'universities') {
                contactInfo.style.display = 'block';
            } else {
    ...
    function displayUniversities(searchQuery = '') {
        ...
        universitiesList.innerHTML += filteredUniversities.map(university => `
    ...
    window.showUniversityDetail = function(universityId) {
        ...
        const reviews = universityReviewsData.filter(r => r.universityId === universityId);
    ...
    window.showProfessorDetail = function(professorId) {
        ...
        const reviews = reviewsData.filter(r => r.professorId === professorId);
    ...
    function submitProfessorData(event) {
        ...
        fetch('/submit-review', {
            ...
            body: JSON.stringify({ type: 'add-professor', content: professorData }),
        })
        .then(response => response.json())
        .then(data => {
            if (data.success) {
                ...
                alert('教授の追加に失敗しました。もう一度お試しください。');
            }
        })
        .catch(error => {
            console.error('Error:', error);
